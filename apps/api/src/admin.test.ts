...
describe('profiles from env (1.5)', () => {
  it('binds operator and default profiles and refuses overrides in prod mode', async () => {
    const profiles = profilesFromEnv({
      ...
      // Every regulated template is a draft while the African markets are unconfirmed, and the host
      // refuses a draft without the override — so operator binding is exercised on the light profile.
      OPERATORS: JSON.stringify({ acme: { profile: 'light', origins: ['https://acme.example'] } }),
    });
    ...
    expect(acme.session.profile.name).toBe('light');
    expect(acme.session.config.id).toBe('whack-crash/v4');
    const override = await call(app, 'POST', '/v1/sessions', undefined, { profile: 'ng-draft' });
  ...
  it('allows the dev override and forbids it in production', () => {
    ...
    expect(profileFromTemplate('ng-draft').operatorOrigins).toEqual([]);
  });
});
