...
// Builds Paper Route placeholder audio: synthesizes PCM, encodes MP3 (lamejs) and WebM/Opus (headless Chrome
// WebCodecs), and writes public/assets/audio/audio.json. Adapted from apps/whack/scripts/build-audio.mjs.
import { Mp3Encoder } from '@breezystack/lamejs';
import { mkdirSync, writeFileSync } from 'node:fs';
import { createRequire } from 'node:module';
import { dirname, join } from 'node:path';
import { chromium } from 'playwright-core';
import { ENGINE_LOOP_SECONDS, SR, engineLoop, sfxBet, sfxLand, sfxReturn, sfxSplash, sfxThrow, sfxTick, sfxWin, sfxWipeout } from './synth.mjs';
...
// --- sfx sprite ---
const GAP = 0.25;
const effects = { tick: sfxTick(), bet: sfxBet(), throw: sfxThrow(), land: sfxLand(), return: sfxReturn(), win: sfxWin(), wipeout: sfxWipeout(), splash: sfxSplash() };
...
const files = {
  ...
  engine: { pcm: doubled(engineLoop()), kbps: 32 },
};
...
const manifest = {
  ...
  engine: { src: src('engine'), loop: loopRegion(ENGINE_LOOP_SECONDS) },
};
