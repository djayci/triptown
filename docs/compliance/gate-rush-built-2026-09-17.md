...
| No session clock or net position strip | PASS for Nigeria and Ghana; a decision to revisit per market | Removed at the user's request (17 Sep 2026) by setting `showSessionClock` and `showNetPosition` false in `ng-draft` and `gh-draft`. It applies to every game on those markets. No Nigerian or Ghanaian rule requiring either was found (`night-meet-2026-09-16.md` s.3). They remain on in the UK (RTS 2E, 13A–C), Ontario (2.21) and Portugal profiles, where they are required. The operator may still show session time in its own wrapper |
