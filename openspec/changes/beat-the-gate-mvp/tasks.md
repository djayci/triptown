...
- [x] 5.1 Create `@triptown/gate`: Vite + PixiJS v8 on `@triptown/crash-client`, own catalogue via `setTranslator`, `Logo('BEAT THE', 'GATE')`, `services.ts` with the mock only under `VITE_DEMO`, `build:demo` and a DEMO label. Verify `pnpm --filter @triptown/gate build` passes `check-no-mock.mjs`.
  - Done 2026-09-17: `apps/gate` runs on the shared controller and `CrashScreen` with its own catalogue and wordmark. The production build reports "No mock code in 15 bundle files"; the mock chunk is present in `dist-demo`. The DEMO badge comes from the shared screen (crash-client 7131e68).
- [x] 5.2 Port the canvas art into an atlas pipeline: horse (gallop, canter, standing), adult jockey, yard gate (open, shut), barn, floodlights, crowd band and turf tiles. Verify the atlas is committed with an art checklist against the adult-art requirement.
  - Done 2026-09-17 (placeholder art): 12-frame atlas committed from `art/art.mjs`; checklist in `apps/gate/art/CHECKLIST.md`. The outline-style risk stays open for task 3.1.
- [x] 5.3 Build the stage: night field with looping parallax, the horse's lap on a fixed-period loop, a static gate, and crowd flashes and meter driven by the multiplier behind `intensityEffects`. Verify a test renders two rounds reaching x4.00 with different crash times and asserts identical scene state at that multiplier.
  - Done 2026-09-17: `GateStage` receives only the multiplier through the shared `GameStage` interface, so it cannot see the crash time by construction. `stage.test.ts` pins that intensity is a pure function of the multiplier and the lap repeats every `LAP_SECONDS`.
...
- [x] 5.5 Wire rounds through the shared controller and view once `the-lift-mvp` 2.3/2.4 land, with no result-kind, disarm or countdown logic in `apps/gate`. Verify `presentation-check.mjs` and `timing-check.mjs` pass for the gate demo, and `grep resultKind apps/gate/src` finds nothing.
  - Done 2026-09-17: `GateView` is the shared `CrashScreen` plus two scene cues. `grep resultKind apps/gate/src` finds nothing. timing-check under ng-draft passed (worst gap 5,131 ms, hold-to-repeat false), evidence `docs/compliance/evidence/2026-09-17/beat-the-gate-timing-check.json`. presentation-check passes under `light` (win celebrated; below-stake and crash not); under ng-draft it also passes: win celebrated, crash not, and the below-stake and even cases are reported UNREACHABLE, because setbacks are off and the minimum cash-out is x1.01. Evidence: `beat-the-gate-presentation-check.json`.
- [x] 5.6 Add the message catalogue with the gate rules text, decoration statement and withholding notice. Verify `copy-check.mjs` passes and fails on a fixture containing race, odds, starting gate, finish or bet slip.
  - Done 2026-09-17: the catalogue is in `src/i18n/en.ts`; `pnpm check:copy` passes with `copy-banned.json`. A fixture catalogue containing "odds" and "race" fails. The withholding notice is shown by the shared rules panel and on winning results (crash-client fe11802), with the gate catalogue keys `rules.withholding` and `result.withholding`.
- [x] 5.7 Add audio (gallop loop, gate open, gate slam, ride-home, win) within 1.5 MB with sources in `SOURCES.md`. Verify the audio budget check, and that no sound changes with anything but the multiplier.
  - Done 2026-09-17: synthesized placeholder audio is 981.3 KB of the 1,500 KB budget, sources in `public/assets/audio/SOURCES.md`. IN! plays hoofbeats and a latch, the crash plays a gate slam, and music intensity follows the multiplier (shared controller).
...
- [x] 6.4 Update `AGENTS.md` with `apps/gate`. Verify the layout section matches `apps/` and `packages/`.
  - Done 2026-09-17: `gate/` added to the AGENTS.md layout next to `lift/`.
