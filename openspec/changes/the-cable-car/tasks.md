...
- [ ] 3.9a Include `run.revealChanceLocked` (`{chance}`, `{multiplier}`) in the catalogue: `CrashScreen` reads it when the value locks, so a game without the key renders the raw key on screen during the wait. Verify it resolves through the catalogue rather than falling back, in the built client (design D5)
