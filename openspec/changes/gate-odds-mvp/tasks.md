...
- [ ] 7.2 Build the Gate Rush scene and screens from the canvas:
  ...
  - 23 Sep 2026 (user): the ride home is a fixed 2.5 s from every press (`headingHomeMs`, never below the shared `HEADING_HOME_MS`) with its own anticipation cue (`headingHomeSfx: 'heading'`) in place of the round music, identical for every outcome. presentation-check (ng-draft) PASS with the same heading-home audio for a win and a loss; timing-check PASS, worst gap 5,021 ms.
