...
8. **Transport is HTTP + SSE only.** No WebSockets, because everything has to fit inside Vercel serverless functions.

## Compliance (certification and licensing)

These games are built to be certified by accredited test labs and licensed to regulated operators. The audits are **`docs/compliance/whack-crash-2026-09-15.md`** (GLI-19, the UK, EU/offshore jurisdictions, the Americas and Africa) and **`docs/compliance/paper-route-2026-09-15.md`** (partial cash-out, rounding, theme). Shared fixes land through the `compliance-baseline` OpenSpec change. To audit any game, run the **`game-compliance-audit`** skill (`.claude/skills/game-compliance-audit/`). Its `references/jurisdictions.md` holds the dated rules and the watch list. Everything here is research, not legal advice.
...
9. **Rounding must not be one-way or break the published RTP.** GLI-19 §4.7.1(a) requires the minimum RTP at any single bet level. A game with several credits per round accrues exact values and rounds once per round, sets a minimum paper or stake value, and publishes RTP at the minimum bet. Nevada Notice 2026-14 bans "only round down" (by analogy).
...
- **The Candy art direction** is a minors-appeal risk in regulated markets. So is the **Paper Route paperboy and dog**: replaced by an adult courier and no animal. Since `paper-route-iso-look` the courier rides a **utility delivery bicycle**, not a moped, so the adult read rests on build, helmet, high-visibility clothing and the cargo bike. The re-audit must confirm that (see `docs/compliance/evidence/2026-09-16/paper-route-art-checklist.md`); the fallback is the same rider on a moped.
